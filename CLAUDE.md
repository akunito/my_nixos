...
**ALWAYS use `install.sh` — NEVER use bare `nixos-rebuild switch`:**

`nixos-rebuild switch` without `install.sh` will use the WRONG `hardware-configuration.nix` (from whichever machine last committed it), causing boot failures, emergency mode, or bricked systems. This has happened in production — it is not theoretical.
...
ssh -A -p 56777 akunito@<VPS-IP> "cd ~/.dotfiles && git fetch origin && git reset --hard origin/main && ./install.sh ~/.dotfiles <PROFILE> -s -u -d"
...
- **CRITICAL: NEVER run `sudo nixos-rebuild switch --flake .#PROFILE --impure` directly on remote machines. ALWAYS use `install.sh` which handles hardware-config regeneration first.**
