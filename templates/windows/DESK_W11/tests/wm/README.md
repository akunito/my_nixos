...
| `rules` | `rules-test.ahk` | The rules ported from `swayfx-config.nix`: the calculator and the file manager float and are sticky, an app with no rule tiles |

## Things that bite when writing cases here

- **AHK names are case-insensitive**: a function called `R()` cannot be used as
  `r := R(x)` -- the parser reads the local `r`. And `GoTo` is a keyword, so it
  can never be a function name. Both fail as a modal error dialog, i.e. as a
