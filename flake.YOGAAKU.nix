  ...
  outputs = inputs@{ self, ... }:
    ...
    let
      ...
      systemSettings = {
        ...
        # Network
        networkManager = true;
        ipAddress = "192.168.8.93"; # ip to be reserved on router by mac (manually)
        wifiIpAddress = "192.168.8.94"; # ip to be reserved on router by mac (manually)
