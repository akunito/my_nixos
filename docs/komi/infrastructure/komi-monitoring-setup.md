...
KOMI_LXC_monitoring (192.168.1.12, CTID 112) runs Grafana and Prometheus as native NixOS modules. It scrapes metrics from all other KOMI_LXC containers.
...
1. SSH to Proxmox: `ssh root@192.168.1.3`
...
|--------|------|------|---------|
| komi_database (node) | 192.168.1.10 | 9100 | System metrics |
| komi_database (postgres) | 192.168.1.10 | 9187 | PostgreSQL metrics |
| komi_database (redis) | 192.168.1.10 | 9121 | Redis metrics |
| komi_mailer (node) | 192.168.1.11 | 9100 | System metrics |
| komi_mailer (cadvisor) | 192.168.1.11 | 9092 | Docker metrics |
| komi_proxy (node) | 192.168.1.13 | 9100 | System metrics |
| komi_proxy (cadvisor) | 192.168.1.13 | 9092 | Docker metrics |
| komi_tailscale (node) | 192.168.1.14 | 9100 | System metrics |

Verify targets: `http://192.168.1.12:9090/targets`
...
prometheusRemoteTargets = [
  # ... existing targets ...
  { name = "new_service"; host = "192.168.1.XX"; nodePort = 9100; cadvisorPort = 9092; }
];
```
...
### Email Alerts
Prometheus alerting rules send notifications via the mailer container (192.168.1.11).
