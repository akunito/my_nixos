...
Documentation for Komi's LXC infrastructure running on Proxmox (192.168.1.3).
...
These containers currently use `192.168.1.x` addresses (akunito's network). Once setup is complete, they will be migrated to `192.168.1.x` (Komi's home network). See the deployment plan for migration steps.
