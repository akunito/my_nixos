...
- **`${hyper}+s`** → sway-apps GUI: rules, shortcuts, tools, nodes, docker, monitoring (only when `swayAppsEnable = true`; state in `user/wm/sway/apps/`). Was `${hyper}+Shift+n` until 2026-09-07; `${hyper}+Shift+n` is free again.
