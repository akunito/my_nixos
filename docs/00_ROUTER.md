...
| user-modules.sway-output-layout-kanshi | Fix “phantom OFF monitors” in Sway/SwayFX using kanshi (wlroots) + Sway-only systemd target, profile-scoped in flakes. | sway, swayfx, wayland, kanshi, outputs, monitors, home-manager, systemd-user, plasma6, profiles | user/wm/sway/kanshi.nix |
