...
| docs.proxmox-lxc | Guide to managing Proxmox LXC containers using a Base + Override pattern. Explains how to create and install new container profiles while keeping configuration DRY. | proxmox, lxc, virtualization, profiles, modularity, dry | docs/proxmox-lxc.md |
...
| user-modules.gaming | Implementation details for Gaming on NixOS, covering Lutris/Bottles wrappers, Vulkan/RDNA 4 driver fixes, and Wine troubleshooting. | gaming, lutris, bottles, wine, vulkan, amd, rdna4, wrappers | user/app/games/games.nix |
