...
This Home Manager module generates standard database credential files for passwordless CLI access to the VPS database server.
...
in
{
  systemSettings = {
    ...
    dbCredentialsHost = "100.64.0.6"; # VPS database server (Tailscale IP)
...
| `dbCredentialsHost` | string | `"100.64.0.6"` | Database server IP |
...
100.64.0.6:5432:plane:plane:<password>
100.64.0.6:6432:plane:plane:<password>
100.64.0.6:5432:rails_database_prod:liftcraft:<password>
100.64.0.6:6432:rails_database_prod:liftcraft:<password>
```
...
psql -h 100.64.0.6 -U plane -d plane

# Or use shell alias
psql-plane

# Via PgBouncer (connection pooling)
psql -h 100.64.0.6 -p 6432 -U plane -d plane
```
...
1. Verify Redis is running on VPS:
   ```bash
   ssh -A -p 56777 akunito@100.64.0.6 "systemctl status redis"
   ```
