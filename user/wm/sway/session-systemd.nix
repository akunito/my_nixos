...

{
    ...
    # - add robustness: kill orphans before start, verify IPC socket, ensure clean termination
    systemd.user.services.waybar = let
      # Script to kill orphan waybar processes and verify Sway IPC is ready
      check-sway-ready = pkgs.writeShellScript "check-sway-ready" ''
        # Kill any orphan waybar processes first (prevents duplicates)
        ${pkgs.procps}/bin/pkill -9 waybar 2>/dev/null || true
        sleep 0.2

        # Wait for Sway IPC socket to be ready (max 6 seconds)
        # This fixes the workspaces widget missing after fast relog
        i=0
        while [ "$i" -lt 30 ]; do
          if [ -S "''${SWAYSOCK:-}" ]; then
            # Verify socket actually responds
            if ${pkgs.sway}/bin/swaymsg -t get_version >/dev/null 2>&1; then
              exit 0
            fi
          fi
          i=$((i + 1))
        ...
        echo "WARNING: Sway IPC not ready after 6s, starting waybar anyway" >&2
        exit 0  # Don't fail - let waybar try
      '';
    in {
      Unit = {
