...
let
  ...
  # sway-apps (systemSettings.swayAppsEnable): when ON the app rules below are
  # NOT emitted and the config includes ~/.config/sway/sway-apps.conf instead,
  # generated by the tool from user/wm/sway/apps/*.json. See ./sway-apps/.
  swayApps = systemSettings.swayAppsEnable or false;
  swayAppsBin = "${config.home.homeDirectory}/.nix-profile/bin/sway-apps";
...
in
{
  ...
  wayland.windowManager.sway = {
    ...
    config = {
      ...
      # Window rules (legacy, superseded by sway-apps when swayAppsEnable = true)
      window = {
        commands = lib.optionals (!swayApps) [
    ...
    extraConfig = ''
      ...
      # With sway-apps on, the pins come from its monitors table instead
      # (user/wm/sway/apps/<profile>.json) and are part of the include above.
      ${lib.optionalString (!swayApps) (lib.concatStringsSep "\n" (
        lib.concatMap (
          p:
      ...
      # sway-apps: dynamic window rules + assignments (GUI/CLI managed, repo-backed).
      # The file is regenerated by Home Manager activation and by the tool itself.
      ${lib.optionalString swayApps "include ~/.config/sway/sway-apps.conf"}

      ${lib.optionalString (!swayApps) ''
