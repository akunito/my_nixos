...
EOF
  ...
  sway-session-start = pkgs.writeShellScriptBin "sway-session-start" ''
    ...
    # exists and before %t/sway-session.env is written, causing Sway-only services (e.g. SwayBG+
    # wallpaper restore) to race and skip.
    #
  ...
  # NOTE: `xkb_numlock` is a **config-only** directive in Sway.
  # It cannot be toggled via IPC (`swaymsg`) after startup.
