...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, app: Adw.Application, ctl: Controller) -> None:
        ...
        # ---- Tools group: launchers in the SAME scrolling column as the sections,
        # separated only by the TOOLS title (a separate tiny scroll area was hard to use)
        tools_hdr = Gtk.Label(label="TOOLS", xalign=0)
        tools_hdr.add_css_class("sa-section-title"); tools_hdr.set_margin_start(20); tools_hdr.set_margin_top(14); tools_hdr.set_margin_bottom(4)
        ...
        column.append(self.nav); column.append(tools_hdr); column.append(self.tools_box)
        scrolled_side = Gtk.ScrolledWindow(vexpand=True)
        scrolled_side.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
        scrolled_side.set_child(column)
        side.append(scrolled_side)
