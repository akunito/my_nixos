...
if systemSettings.stylixEnable == true then {
  ...
  home.file = lib.mkMerge [
    {
      ...
      ".stylix-debug.log".text = ''
        ...
        qt5ctFilesRemoved: ${toString (userSettings.wm == "plasma6" && systemSettings.enableSwayForDESK == false)}
      '';
  ...
  # CRITICAL: qt5ct File Management Strategy
  # 
  # When enableSwayForDESK = false:
  #   - qt5ct files are removed via home.file above (empty text overrides Stylix-generated files)
  #   - Plasma 6 uses native Breeze theme (QT_QPA_PLATFORMTHEME is unset)
  #
  # When enableSwayForDESK = true:
  #   - qt5ct files are generated by Stylix (Qt target is enabled)
  ...
  #
  # CRITICAL: Backup qt5ct files to allow restoration if Plasma 6 modifies them
  ...
  home.activation.backupAndProtectQt5ct = lib.hm.dag.entryAfter ["writeBoundary"] ''
    ...
    if [ "${toString (systemSettings.enableSwayForDESK == true)}" = "true" ]; then
      ...
      # Protection against Plasma 6 modifications is provided by restoration on Sway startup
      echo "qt5ct files backed up (writable to allow Dolphin preferences)"
    fi
  '';
} else {
