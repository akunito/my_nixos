  ...
  alacritty-tmux-wrapper = pkgs.writeShellScriptBin "alacritty-tmux-wrapper" ''
    ...
    while [ "$i" -lt 30 ]; do
      ...
      if [ -n "$SESSIONS" ]; then
        SESSION="$(printf '%s\n' "$SESSIONS" | tail -n 1)"
        if [ -n "$SESSION" ]; then
          log "attach last session=$SESSION"
    ...
    # No sessions yet: attach a bootstrap session to trigger restore hook
    BOOTSTRAP="__bootstrap"
    if $TMUX has-session -t "$BOOTSTRAP" 2>/dev/null; then
      log "attach bootstrap (existing)"
      exec $TMUX attach -t "$BOOTSTRAP"
    fi
    log "no restored sessions; creating bootstrap session"
    if $TMUX new-session -d -s "$BOOTSTRAP" 2>/dev/null; then
      log "attach bootstrap (new-session)"
      exec $TMUX attach -t "$BOOTSTRAP"
    fi
    log "bootstrap session failed exit=$?"
