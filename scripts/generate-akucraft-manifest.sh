...
## The two survival worlds

There are **two survival worlds** here, not one, plus the usual Nether and End.

| | Overworld | The frontier |
|---|---|---|
| Dimension id | \`minecraft:overworld\` | \`multiworld:frontier\` |
| Border | 12,000 blocks from spawn, every direction | 30,000 |
| Live 3D map | \`world\` | \`frontier\` — same BlueMap, map picker top-right |

The frontier is a second, wilder survival world created with **Multiworld** and
generated by **Terralith**. It is not a copy of the Overworld and not a creative
world: it has its own terrain, its own structures, its own day/night cycle, its
own land claims and its own border. Graves, beds, waystones and skills all work
there the way they do at home.

**Getting there.** Walk through the stone arch on the Overworld coast at
\`-151 71 54\`. It drops you at the matching arch in the frontier at
\`-1467 150 -183\`; walking back through that one brings you home. That arch is
the only route. It is not a Nether portal, it is not made of obsidian, and
lighting obsidian will not build you another one.

**What travels with you.** The gateway is a datapack teleport, so what comes
along is exactly what it is written to pick up:

- **You**, always.
- **Your own tamed wolves, cats, parrots and Doggy Talents dogs** standing
  within 16 blocks — provided they are **not sitting**. A sitting pet stays
  behind on purpose: *sit* means stay, in every world. Somebody else's pet is
  never taken, only your own. *(Added 2026-08-22, after komi tested it and her
  wolf was left behind.)*
- **Nothing else.** A mount you are riding, leashed animals, boats, minecarts,
  item frames and dropped items all stay where they are. Dismount, unleash, and
  carry what you need in your inventory.

Doggy Talents also has its own way across: a dog with **Cross Origin Teleport**
enabled can be summoned to you from another dimension with the **Conducting
Bone** (\`conducting_bone_cross_origin\` is on). Dogs are stopped from wandering
into portals by themselves (\`all_dog_block_portal\`), which is why they never
