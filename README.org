...
** Additional user (multiples users on same the same system)
I have implemented a way to install an additional user with its own home.nix configuration for home-manager
