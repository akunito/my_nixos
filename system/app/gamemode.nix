...

{
  ...
  # Configured to avoid AMDGPU conflicts on RDNA3 (7800 XT) by disabling GPU optimizations
  ...
  programs.gamemode = lib.mkIf (systemSettings.gamemodeEnable == true) {
    ...
    settings = {
      ...
      gpu = {
        # CRITICAL: Completely disable GPU optimizations to avoid crashes on RDNA3 (7800 XT)
