...

{
  ...
  fileSystems = lib.mkMerge [
    (lib.mkIf (systemSettings.disk1_enabled) {
      "${systemSettings.disk1_name}" = lib.mkForce {
    ...
    (lib.mkIf (systemSettings.disk2_enabled) {
      "${systemSettings.disk2_name}" = lib.mkForce {
    ...
    (lib.mkIf (systemSettings.disk3_enabled) {
      "${systemSettings.disk3_name}" = lib.mkForce {
    ...
    (lib.mkIf (systemSettings.disk4_enabled) {
      "${systemSettings.disk4_name}" = lib.mkForce {
    ...
    (lib.mkIf (systemSettings.disk5_enabled) {
      "${systemSettings.disk5_name}" = lib.mkForce {
    ...
    (lib.mkIf (systemSettings.disk6_enabled) {
      "${systemSettings.disk6_name}" = lib.mkForce {
    ...
    (lib.mkIf (systemSettings.disk9_enabled) {
      "${systemSettings.disk9_name}" = lib.mkForce {
        device = systemSettings.disk9_device;
        fsType = systemSettings.disk9_fsType;
        options = systemSettings.disk9_options;
      };
    })
  ];
}
