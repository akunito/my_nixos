...
# VMs (VMDESK) are automatically excluded - hypervisor handles I/O scheduling
lib.mkMerge [
  # Desktop I/O scheduler (DESK, AGADESK)
  ...
  # Laptop I/O scheduler (LAPTOP, YOGAAKU)
  ...
  # Server I/O scheduler (VMHOME, HOME)
  ...
  # VMs (VMDESK with hostname "nixosdesk") are automatically excluded
  # Hypervisors handle I/O scheduling for virtual machines, so we don't set schedulers
  # This is intentional - no rules match VMDESK, so it gets no I/O scheduler changes
]
