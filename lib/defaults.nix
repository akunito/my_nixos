...

{
  systemSettings = {
    ...
    # share whose server stopped answering is now unconditional for every NFS
    # client (system/hardware/nfs_client.nix) — it is a safety net, not a feature.
    nfsAutoMounts = [ ];
    ...
    gpuMemSamplerEnable = false; # Log AMD VRAM/GTT/MemAvailable to the journal periodically — confirms the suspend GTT ratchet stays fixed (see system/hardware/gpu-mem-sampler.nix)
    gpuMemSamplerIntervalSec = 60; # Sampling cadence in seconds
    ...
    financeUser = ""; # Finance Tagger Flask app login username (set from secrets/domains.nix)
    financePassword = ""; # Finance Tagger Flask app login password (set from secrets/domains.nix)
