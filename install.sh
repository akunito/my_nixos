...
if [ ${#POSITIONAL_ARGS[@]} -gt 1 ]; then
    ...
    echo "  -h|--skip-hardware: Skip hardware-configuration.nix generation (REFUSED if the committed config belongs to another machine — a fresh install on new hardware needs --force)"
    echo "  -q|--quick: Shorthand for -d -h (backward compatibility)"
    echo "  -f|--force: Bypass the ENV_PROFILE safety check AND the -h hardware-config check (first-time installs)"
